...
namespace YooAsset.Editor
{
    ...
    public class AssetBundleCollectorSetting : ScriptableObject
    {
        ...
        public CollectResult BeginCollect(string packageName, bool simulateBuild, bool useAssetDependencyDB)
        {
            ...
            //! 创建资源收集命令（其实就是一次“收集会话”的上下文与开关集合，这些开关被 Collector、Group、Package 的逻辑用来决定具体收集行为。）
            ...
            //! 预热依赖数据库：即使模拟构建忽略 GetDependencies，也强制生成/更新缓存文件
            //if (useAssetDependencyDB)
            //{
            //    _ = command.AssetDependency; // 触发 AssetDependencyCache(true) 构造，内部会 SaveDatabase()
            //}

...
namespace YooAsset.Editor
{
    internal abstract class BuildPipelineViewerBase
    {
        ...
        /// <summary>
        /// 创建资源包加密服务类实例。
        /// 从 EditorPrefs 读取类名，反射找到该 Type 并 Activator.CreateInstance 实例化 IEncryptionServices
        /// </summary>
        ...
        /// <summary>
        /// 用 EditorTools.GetAssignableTypes(typeof(IEncryptionServices)) 反射收集所有实现 IEncryptionServices 的类型，作为下拉框列表。
        /// 选择变化时写入 EditorPrefs：AssetBundleBuilderSetting.SetPackageEncyptionServicesClassName(...)
        /// <para>默认值：AssetBundleBuilderSetting.GetPackageEncyptionServicesClassName(...) 返回 EditorPrefs 中保存的类型名，若无则默认为 typeof(EncryptionNone).FullName</para>
        /// </summary>

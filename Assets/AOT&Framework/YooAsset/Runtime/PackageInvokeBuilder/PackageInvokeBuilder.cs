...
namespace YooAsset
{
    public static class PackageInvokeBuilder
    {
        ...
        {
            var assemblyName = buildParam.InvokeAssmeblyName;   // "YooAsset.Editor"
            var className = buildParam.InvokeClassFullName;     // "YooAsset.Editor.AssetBundleSimulateBuilder"
            var methodName = buildParam.InvokeMethodName;       // "SimulateBuild"
            //? 这里为什么不用Activator.CreateInstance()，是因为我们只有字符串吗？
